...
class TrackData {
  ...
  CompletedTrackShowPlacement(playerProfile: PlayerProfile) {
    playerProfile.PlacementUI?.Open(
      mod.stringkeys.header_placement,
      currentRace.playersInRace.findIndex(
        (pp) => pp.player == playerProfile.player
      ) + 1,
      45
    );
  }
  ...
  hasActiveRacers() {
    return currentRace.playersInRace.some(
      (player) => player.completedTrack === false
    );
  }
  ...
  async AddPlayerToTrack(player: mod.Player) {
    ...
    if (playerP) {
      ...
      playerP.checkPointPosition = ConverVector3ToModVector(
        currentRace.checkPoints[0].position
      );
  ...
  PlayerLeftGame() {
    ...
    currentRace.playersInRace = currentRace.playersInRace.filter(
      (playerProfile) => {
  ...
  async RaceStartCountdown() {
    currentRace.playersInRace.forEach((playerProfile) => {
      playerProfile.StartCountDownUI?.Open(
        mod.stringkeys.scoreboard_1_name,
    ...
    while (this.countdownToStart > 1) {
      ...
      currentRace.playersInRace.forEach((playerProfile) => {
        playerProfile.StartCountDownUI?.update(
          mod.stringkeys.scoreboard_1_name,
          this.countdownToStart
        );
      });
  ...
  IsPlayerInRace(player: mod.Player) {
    ...
    return (
      currentRace?.playersInRace?.some(
  ...
  async StartGame() {
    ...
    this.playersInRace.forEach((playerProfile) => {
      ...
      if (
        mod.IsPlayerValid(playerProfile.player) &&
  ...
  async DistanceCheckToCheckpoint() {
    while (
      this.trackState == TrackState.running ||
      ...
      this.playersInRace.forEach((playerProfile) => {
        ...
        const targetCheckpoint = ConverVector3ToModVector(
          this.checkPoints[playerProfile.nextCheckpoint].position
        );
        ...
        if (distance <= playerProfile.checkpointCircleSize) {
          ...
          playerProfile.checkpointCircleSize = getCheckpointSize(
            positionInRace,
            currentRace.playersInRace.length
          );

          //update checkpoint position
          playerProfile.nextCheckpoint =
            playerProfile.checkpoint % this.checkPoints.length;
          playerProfile.checkPointPosition = ConverVector3ToModVector(
            this.checkPoints[playerProfile.nextCheckpoint].position
          );

          const lapsCompl = Math.floor(
            playerProfile.checkpoint / this.checkPoints.length
          );
  ...
  StartFireworks() {
    const fireworkArray = generateSpawnLine(
      { x: 121.062789916992, y: 177.110992431641, z: 163.706634521484 },
      { x: 277.184600830078, y: 177.110992431641, z: 13.0980796813965 },
      6
    );

    fireworkArray.forEach((firework) => {
      SpawnVFXAtPosition(
        ConverVector3ToModVector(firework.position),
  ...
  UpdateOrder() {
    ...
    if (hasOrderChangedByKey(oldarray, updatedArray)) {
      ...
      if (
        currentRace.playersInRace.length >= 2 &&
        catchupMechanicSprintDisable
      ) {
        ...
        currentRace.playersInRace.length <= 1 &&
        catchupMechanicSprintDisable
      ) {
...
function hasOrderChangedByKey(
  prev: PlayerProfile[],
  current: PlayerProfile[]
): boolean {
  if (prev.length !== current.length) return true;
  return prev.some((p, i) => p.playerProfileId !== current[i].playerProfileId);
}
...
class PlayerProfile {
  ...
  InitRacer() {
    this.checkPointPosition = ConverVector3ToModVector(
      currentRace.checkPoints[0].position
    );
    this.checkPointPosition2 = currentRace.checkPoints[0].position;

    this.checkpointWorldIconsHolder = new HoH_CheckpointWorldIconsHolder(
      this,
      mod.CreateVector(0, 1, 0)
    );
    this.nextcheckpointWorldIconsHolder = new HoH_CheckpointWorldIconsHolder(
      this,
  ...
  RemoveWorldIcons() {
    if (this.checkpointWorldIconsHolder) {
      this.checkpointWorldIconsHolder.checkpointWorldIcons.forEach(
        (element) => {
    ...
    if (this.nextcheckpointWorldIconsHolder) {
      this.nextcheckpointWorldIconsHolder.checkpointWorldIcons.forEach(
        (element) => {
          mod.UnspawnObject(element);
        }
      );
  ...
  UpdateWorldIconPosition() {
    ...
    this.checkPointPosition2 =
      currentRace.checkPoints[this.nextCheckpoint].position;

    this.checkPointPositionLookDirection2 =
      currentRace.checkPoints[
        (this.checkpoint + 1) % currentRace.checkPoints.length
      ].position;
    ...
    this.checkpointWorldIconsHolder?.Update(
      this.checkPointPosition2,
      this.checkPointPositionLookDirection2,
      this.checkpointCircleSize
    );

    //upcoming checkpoint after target checkpoint
    const upcomingCheckpoint =
      (this.checkpoint + 1) % currentRace.checkPoints.length;
    const upcomingcheckpointPosition2 =
      currentRace.checkPoints[upcomingCheckpoint].position;

    this.nextcheckpointWorldIconsHolder?.Update(
      upcomingcheckpointPosition2,
      currentRace.checkPoints[
        (this.checkpoint + 2) % currentRace.checkPoints.length
      ].position,
      this.checkpointCircleSize
    );
  ...
  static removePlayer(player: mod.Player) {
    ...
    if (hohPlayer) {
      ...
      if (debugPlayer)
        console.log("Error: could not find player with profile to remove");
    }
  }

  static removeInvalidPlayers() {
    // Remove invalid from array
    PlayerProfile.playerInstances = PlayerProfile.playerInstances.filter(
      (player) => this.isValidPlayer(player)
    );
...
class HoH_CheckpointWorldIconsHolder {
  ...
  constructor(playerprofile: PlayerProfile, color: mod.Vector) {
    ...
    const checkpointWidgets = generatePointsInACircle(
      this.#playerprofile.checkPointPosition2,
      this.#playerprofile.checkPointPositionLookDirection2,
      this.circleSize,
      this.amountoficons
    );
    ...
    checkpointWidgets.forEach((element) => {
      const worldicon = mod.SpawnObject(
        ...
        mod.CreateVector(element.x, element.y, element.z),
  ...
  Update(
    ...
    nextcheckpointPosition: Vector3,
    circleSize: number = this.circleSize
  ) {
...
async function SpawnVFXAtPosition(
...
class VehicleHandler {
  ...
  static RequestVehicle(playerProfile: PlayerProfile) {
    if (
      ...
      VehicleHandler.playerNeedingVehicle.push({
        ...
        position: ConverVector3ToModVector(targetSpawnPoint.position),
  ...
  static async OnVehicleSpawned(eventVehicle: mod.Vehicle) {
    ...
    if (targetVehicleSpawner == undefined) {
      console.log(
        "Could not find a vehicle close enough to the vehicle spawnpoint"
      );
      return;
    }

    const index =
      VehicleHandler.playerNeedingVehicle.indexOf(targetVehicleSpawner);
    if (index !== -1) {
      VehicleHandler.playerNeedingVehicle.splice(index, 1);
    }

    while (
      currentRace.IsPlayerInRace(targetVehicleSpawner.player) &&
      !mod.GetSoldierState(
        targetVehicleSpawner.player,
        mod.SoldierStateBool.IsAlive
      )
    ) {
      await mod.Wait(0.1);
...
function GetUnoccupiedVehicleInRange(pos: mod.Vector): mod.Vehicle | undefined {
  ...
  for (let index = 0; index < mod.CountOf(allVeh); index++) {
    ...
    if (!mod.IsVehicleOccupied(veh)) {
      const vehPos = mod.GetVehicleState(
        veh,
        mod.VehicleStateVector.VehiclePosition
      );
...
export async function OnPlayerUIButtonEvent(
  ...
  playerProfile.VehicleShopUI?.OnButtonPressed(
    eventPlayer,
    eventUIWidget,
    eventUIButtonEvent
  );
...
const tracks: RaceTrack[] = [
  ...
  },
  {
    ...
    checkPoints: [
      ...
      {
        id: 108,
        checkpointStart: { x: 1174.2579345, y: 187.29, z: 1043.66088 },
        checkpointEnd: {
          x: 1047.197265625,
          y: 186.30224609375,
          z: 1181.48742675781,
        },
        position: { x: 1552.362, y: 201.8742, z: 1429.677 },
      },
      {
        id: 109,
        checkpointStart: { x: 1174.2579345, y: 187.29, z: 1043.66088 },
        ...
        position: { x: 2048.393, y: 208.4673, z: 1706.873 },
      },

      {
        ...
        position: { x: 2676.97, y: 208.4673, z: 1946.0 },
      },
      {
        id: 111,
        checkpointStart: {
...

      {
        ...
        position: { x: 3939.844, y: 263.038, z: 2924.436 },
      },
      {
        id: 113,
        checkpointStart: {
...

      {
        ...
        position: { x: 5288.721, y: 355.434, z: 3896.095 },
      },
      {
        id: 115,
        checkpointStart: {
        ...
        position: { x: 5466.977, y: 423.387, z: 3535.378 },
      },

      {
        ...
        position: { x: 5326.602, y: 557.992, z: 3181.616 },
      },
      {
        id: 117,
        checkpointStart: {
...

      {
        ...
        position: { x: 3970.04, y: 334.594, z: 2572.894 },
      },
      {
        id: 119,
        checkpointStart: {
        ...
        position: { x: 3342.115, y: 260.9633, z: 2219.49 },
      },

      {
        ...
        position: { x: 2624.016, y: 293.6768, z: 2070.868 },
        flipdir: true,
      },
      {
        id: 121,
        ...
        position: { x: 2050.632, y: 311.7585, z: 1620.939 },
        flipdir: true,
      },

      {
        ...
        position: { x: 1518.23, y: 323.8511, z: 1482.588 },
        flipdir: true,
      },
      {
        id: 122,
      ...
      },
      {
        ...
        position: { x: -530.65, y: 301.9926, z: -101.325 },
        flipdir: true,
      },
      {
        id: 125,
        ...
        position: { x: -1235.055, y: 251.483, z: -262.1271 },
        flipdir: true,
      },

      {
        ...
        position: { x: -2510.07, y: 675.4128, z: -367.911 },
      },
      {
        id: 127,
        checkpointStart: {
        ...
        position: { x: -3051.823, y: 992.7201, z: -134.6379 },
      },
      {
        id: 1280,
        checkpointStart: {
        ...
        position: { x: -3324.713, y: 924.931, z: 221.4012 },
      },

      {
        ...
        position: { x: -3465.781, y: 875.6663, z: 683.3296 },
      },
      {
        id: 129,
        checkpointStart: {
        ...
        position: { x: -3273.344, y: 829.738, z: 973.0317 },
      },
      {
        id: 130,
        checkpointStart: {
        ...
        position: { x: -2918.56, y: 751.904, z: 941.331 },
      },
      ...
      },
      {
        ...
        position: { x: -2477.86, y: 537.821, z: 174.285 },
      },

      {
        ...
        position: { x: -1207.372, y: 354.4803, z: -96.10004 },
      },
      {
        id: 133,
        checkpointStart: {
        ...
        position: { x: -80.90897, y: 260.4618, z: 68.27824 },
      },
    ],
  },
];
...
class HoH_BenjiOvertakeUI {
  ...
  #Create() {
    const coolahhuiname: string =
      "overtake_message_" + this.#playerprofile.playerProfileId;
    mod.AddUIText(
      coolahhuiname,
      mod.CreateVector(0, 100, 0),
      mod.CreateVector(200, 40, 0),
      mod.UIAnchor.TopCenter,
      MakeMessage(mod.stringkeys.overtakenmessage),
    ...
    mod.SetUIWidgetBgColor(widget, mod.CreateVector(0.678, 0.753, 0.8));
  ...
  CreateFadeLineUI(right: boolean): mod.UIWidget {
    const coolahhuiname: string =
      "overtake_message_line_" +
      right +
      "_" +
      this.#playerprofile.playerProfileId;
    let horizontalOffset: number = right ? 175 : -175;
    mod.AddUIContainer(
      coolahhuiname,
      mod.CreateVector(horizontalOffset, 100, 0),
...
class HoH_BoosterDisabledUI {
  ...
  #Create() {
    const coolahhuiname: string =
      "booster_disable_message_" + this.#playerprofile.playerProfileId;
    mod.AddUIText(
      coolahhuiname,
      mod.CreateVector(this.widgetHorisontalPos, this.widgetHeightPos, 0),
      mod.CreateVector(205, 36, 0),
      mod.UIAnchor.BottomCenter,
      MakeMessage(mod.stringkeys.boosterdisabled),
      this.#playerprofile.player
    );
    const widget = mod.FindUIWidgetWithName(coolahhuiname);
...
class HoH_UIEndingGameCountdown {
  ...
  constructor(playerProf: PlayerProfile) {
    ...
    this.textWidget = mod.FindUIWidgetWithName(
      "ending_countdown_text_" + this.#Player.playerProfileId
    );
  }
  ...
  Open(timeleft: number) {
    ...
    this.textWidget &&
      mod.SetUITextLabel(
        this.textWidget,
        MakeMessage(mod.stringkeys.gameending, timeleft)
      );
  }
...
class HoH_UIBlackScreen {
  ...
  Close() {
    this.#black_screen_widget &&
      mod.SetUIWidgetVisible(this.#black_screen_widget, false);
  }
  ...
  async FadeIn() {
    if (this.#black_screen_widget) {
      this.#black_screen_widget &&
        mod.SetUIWidgetVisible(this.#black_screen_widget, true);
...
class HoH_UIPlacementHeader {
  ...
  constructor(playerProf: PlayerProfile) {
    ...
    this.textWidget = mod.FindUIWidgetWithName(
      "placement_text" + this.#Player.playerProfileId
    );
  }
  ...
  Open(text: string, placement: number, scale: number) {
    this.headerWidget && mod.SetUIWidgetVisible(this.headerWidget, true);
    this.textWidget &&
      mod.SetUITextLabel(this.textWidget, MakeMessage(text, placement));
...
class HoH_ScoreboardUI {
  ...
  update() {
    ...
    const playerPositionInRace = currentRace.playersInRace.indexOf(
      this.#playerProfile
    );

    this.#scoreboardPlacement_text &&
      mod.SetUITextLabel(
        this.#scoreboardPlacement_text,
        MakeMessage(
          mod.stringkeys.position_in_race,
          playerPositionInRace + 1,
          currentRace.playersInRace.length
        )
      );
    ...
    for (let index = 0; index < this.#ScoreboardplayerName.length; index++) {
      ...
      if (playerInPos) {
        ...
        if (playerTime) {
          ...
          mod.SetUITextLabel(
            this.#ScoreboardTimeOne[index],
            MakeMessage(
              mod.stringkeys.scoreboard_1_time_1,
              formatTime[0],
              formatTime[1]
            )
          );
          mod.SetUITextLabel(
            this.#ScoreboardTimeTwo[index],
            MakeMessage(
              mod.stringkeys.scoreboard_1_time_2,
              formatTime[2],
              formatTime[3],
              formatTime[4]
            )
        ...
        mod.SetUITextLabel(
          this.#ScoreboardplayerName[index],
          MakeMessage(
            mod.stringkeys.scoreboard_1_name,
            currentRace.playersInRace[index].player
          )
        );
        ...
        if (currentRace.trackState == TrackState.selected) {
          ...
          if (playerProf.readyUp) {
            mod.SetUITextColor(
              this.#ScoreboardplayerName[index],
              mod.CreateVector(0.4196, 0.9098, 0.0745)
            );
            mod.SetUITextColor(
              this.#ScoreboardPlacement[index],
              mod.CreateVector(0.4196, 0.9098, 0.0745)
            );
            mod.SetUITextColor(
              this.#ScoreboardTimeOne[index],
            ...
            mod.SetUITextColor(
              this.#ScoreboardplayerName[index],
              mod.CreateVector(0.678, 0.753, 0.8)
            );
            mod.SetUITextColor(
              this.#ScoreboardPlacement[index],
              mod.CreateVector(0.678, 0.753, 0.8)
  ...
  Create() {
    ...
    const ScoreboardplayerPlacement = `Scoreboard2Placement_text_${
      this.#playerProfile.playerProfileId
    }`;

    const Scoreboardlaps = `Scoreboard2laps_text_${
      this.#playerProfile.playerProfileId
    }`;
    ...
    for (let index = 0; index < MapPlayers; index++) {
      const ScoreboardplayerName = `Scoreboard2playerName${index}text_${
        this.#playerProfile.playerProfileId
      }`;
      const ScoreboardPlacement = `Scoreboard2Placement${index}text_${
        this.#playerProfile.playerProfileId
      }`;
      const ScoreboardTimeOne = `Scoreboard2TimeOne${index}text_${
        this.#playerProfile.playerProfileId
      }`;
      const ScoreboardTimeTwo = `Scoreboard2TimeTwo${index}text_${
        this.#playerProfile.playerProfileId
      }`;
    ...
    this.#scoreboardPlacement_text = mod.FindUIWidgetWithName(
      ScoreboardplayerPlacement
    );
    ...
    for (let index = 0; index < MapPlayers; index++) {
      ...
      this.#ScoreboardPlacement.push(
        mod.FindUIWidgetWithName(ScoreboardPlacement)
      );
      this.#ScoreboardplayerName.push(
        mod.FindUIWidgetWithName(ScoreboardplayerName)
      );
    ...
    //Extra settings
    this.#CoreWidget &&
      mod.SetUIWidgetDepth(this.#CoreWidget, mod.UIDepth.BelowGameUI);
  }
...
class HoH_UIStartCountdown {
  ...
  constructor(playerProf: PlayerProfile) {
    ...
    this.textWidget = mod.FindUIWidgetWithName(
      "start_countdown_text" + this.#Player.playerProfileId
    );
  }
...
class HoH_UIButtonHolder {
  ...
  constructor(
    playerProfile: PlayerProfile,
    uniqueButtonName: string,
    uniqueTextName: string,
    uniqueSelectName?: string,
    clickFunction?: HoH_ClickFunction,
    focusFunction?: HoH_FocusFunction
  ) {
...
class HoH_UIVehicleSelect {
  ...
  constructor(player: PlayerProfile) {
    ...
    this.#LEFT_BUTTON = `left_veh_button_${
      this.#playerProfile.playerProfileId
    }`;
    this.#LEFT_BUTTON_TEXT = `left_veh_button_text_${
      this.#playerProfile.playerProfileId
    }`;
    this.#LEFT_BUTTON_SELECTED = `left_veh_button_selected_${
      this.#playerProfile.playerProfileId
    }`;

    this.#RIGHT_BUTTON = `right_veh_button_${
      this.#playerProfile.playerProfileId
    }`;
    this.#RIGHT_BUTTON_TEXT = `right_veh_button_text_${
      this.#playerProfile.playerProfileId
    }`;
    this.#RIGHT_BUTTON_SELECTED = `right_veh_button_select_${
      this.#playerProfile.playerProfileId
    }`;

    this.#READYUP_BUTTON = `readyup_veh_button_${
      this.#playerProfile.playerProfileId
    }`;
    this.#READYUP_BUTTON_TEXT = `readyup_veh_button_text_${
      this.#playerProfile.playerProfileId
    }`;
    this.#READYUP_BUTTON_SELECT = `readyup_veh_button_select_text_${
      this.#playerProfile.playerProfileId
    }`;

    this.#PLAYERS_READY_COUNT_TEXT = `players_ready_count_text_${
      this.#playerProfile.playerProfileId
    }`;
    this.#CURRENT_SELECT_VEH_TEXT = `current_select_veh_text_${
      this.#playerProfile.playerProfileId
    }`;
    this.#PLAYERS_WAITING_TEXT = `players_waiting_text_${
      this.#playerProfile.playerProfileId
    }`;

    this.selectVehNb = currentRace.availableVehicles.indexOf(
      this.#playerProfile.selectedVehicle
    );
  }

  UIUpdatePlayersReady() {
    const { ready, total } = currentRace.GetPlayersReady();
    this.#players_ready_count_widget &&
      mod.SetUITextLabel(
        this.#players_ready_count_widget,
        MakeMessage(mod.stringkeys.readyplayers, ready, total)
      );

    if (total < MinimumPlayerToStart) {
      this.#players_ready_text_widget &&
        mod.SetUITextLabel(
          this.#players_ready_text_widget,
          MakeMessage(
            mod.stringkeys.waitingforplayersX,
            total,
            MapPlayers,
    ...
    this.#players_ready_text_widget &&
      mod.SetUITextLabel(
        this.#players_ready_text_widget,
        MakeMessage(mod.stringkeys.startsin, currentRace.readyupCountDown)
      );
  ...
  EnableSwitchButtons(enabled: boolean) {
    this.#Left_Button_Holder &&
      mod.SetUIButtonEnabled(this.#Left_Button_Holder.button_widget, enabled);
    this.#Right_Button_Holder &&
      mod.SetUIButtonEnabled(this.#Right_Button_Holder.button_widget, enabled);

    if (enabled) {
      this.#Readyup_Button_Holder &&
        mod.SetUIWidgetBgColor(
          this.#Readyup_Button_Holder.button_widget,
          mod.CreateVector(1, 1, 1)
        );
      this.#Left_Button_Holder &&
        mod.SetUIWidgetVisible(this.#Left_Button_Holder.button_widget, true);
      this.#Right_Button_Holder &&
        mod.SetUIWidgetVisible(this.#Right_Button_Holder.button_widget, true);
    } else {
      this.#Readyup_Button_Holder &&
        mod.SetUIWidgetBgColor(
          this.#Readyup_Button_Holder.button_widget,
      ...
      this.#Left_Button_Holder &&
        mod.SetUIWidgetVisible(this.#Left_Button_Holder.button_widget, false);
      this.#Right_Button_Holder &&
        mod.SetUIWidgetVisible(this.#Right_Button_Holder.button_widget, false);
    }
  }
  ...
  #create() {
        ...
        children: [
          {
            ...
            textColor: bfBlueColor,
            textAnchor: mod.UIAnchor.Center,
            textLabel: MakeMessage(
              mod.stringkeys.startsin,
              currentRace.readyupCountDown
            ),
            textSize: 38,
          },
          {
    ...
    this.#players_ready_text_widget = mod.FindUIWidgetWithName(
      this.#PLAYERS_WAITING_TEXT
    );
    this.#players_ready_count_widget = mod.FindUIWidgetWithName(
      this.#PLAYERS_READY_COUNT_TEXT
    );
    ...
    this.#Left_Button_Holder = new HoH_UIButtonHolder(
      ...
      function (this: HoH_UIButtonHolder, focusIn: boolean) {
        if (focusIn) {
          ...
          this.select_widget &&
            mod.SetUIWidgetVisible(this.select_widget, true);
        } else {
          ...
          this.select_widget &&
            mod.SetUIWidgetVisible(this.select_widget, false);
        }
      }
    );
    ...
    this.#current_select_veh_Widget = mod.FindUIWidgetWithName(
      this.#CURRENT_SELECT_VEH_TEXT
    );

    // CatchupMechanic Information

    const PLAYER_CATCHUP_MECHANIC_TITLE = `players_catchup_mechanic_title_${
      this.#playerProfile.playerProfileId
    }`;
    const PLAYER_CATCHUP_MECHANIC_DESC = `players_catchup_mechanic_desc_${
      this.#playerProfile.playerProfileId
    }`;
  ...
  cameraTeleport() {
    ...
    const spawnPositions = generateSpawnLine(
      startPos,
      endPod,
      MapPlayers,
      "right"
    ...
    const spawnposition =
      spawnPositions[this.#playerProfile.playerRacerNumber].position;

    const lookrotation = this.lookAtYaw(
      spawnposition,
      VehiclePositions[this.selectVehNb]
    );
    mod.Teleport(
      this.#playerProfile.player,
  ...
  refresh() {
    this.#playerProfile.SetVehicle(
      currentRace.availableVehicles[this.selectVehNb]
    );
    const vehicleEnumValue = currentRace.availableVehicles[this.selectVehNb];

    this.#current_select_veh_Widget &&
      mod.SetUITextLabel(
        this.#current_select_veh_Widget,
        MakeMessage(this.getVehName(vehicleEnumValue))
      );
  }
  ...
  async open() {
    if (!this.#players_ready_count_widget) this.#create();
  ...
  buttonFocused(widgetName: string, focusIn: boolean = false) {
    if (widgetName == this.#LEFT_BUTTON && this.#Left_Button_Holder) {
      ...
      widgetName == this.#READYUP_BUTTON &&
      this.#Readyup_Button_Holder
    ) {
